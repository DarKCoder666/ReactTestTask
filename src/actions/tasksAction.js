...
export const loadTasks = (params = { sort, direction, page: 0 }) => (dispatch, getState) => {
...
export const editTask = (unorderedData) => (dispatch) => {
    let params = {
        developer: config.devName,
    };
    ...
    unorderedData.signature = 'trash';
    orderedData.token = 'beejee';

    let encodedObject = {};
    for (let prop in orderedData) {
        encodedObject[fixedEncodeURIComponent(prop)] = fixedEncodeURIComponent(orderedData[prop]);
    }
    ...
    axios.post(config.reqUrl + "edit/" + orderedData.id, orderedData, {
        params: {
            ...orderedData,
            ...params
        }
